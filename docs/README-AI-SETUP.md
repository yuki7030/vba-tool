...
- scripts/check_acceptance.py: SPEC の「受け入れ基準」が機械可読な形か(AC-ID・判定・
  検証手段の3点)を検査。検証コマンドは allowlist + メタ文字 + 親ディレクトリ参照の
  3点で制限する。**本スクリプトはコマンドを実行しない**(実行はエージェントの Bash
  ツール経由に限定し、PreToolUse の block_dangerous_bash.py を必ず通すため)
