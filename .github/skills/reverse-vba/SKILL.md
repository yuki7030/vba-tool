...
2. **as-is を `docs/spec/` に直接書かない。** as-is は観測結果であって合意ではない。
   混ぜると既存挙動が「仕様」に昇格し、AGENTS.md の
   「仕様と既存挙動が矛盾する場合は仕様を正とする」が自己言及で無意味化する。
   正本(`docs/spec/features/`)へ移すには spec-writing スキルの
   「as-is からの昇格」手順(`【推測】` の解消 → 人の承認)を必ず通す。
...
(正本は昇格後の `docs/spec/features/` 側が持つ)。代わりに `manifest.json` が対象ファイルの
